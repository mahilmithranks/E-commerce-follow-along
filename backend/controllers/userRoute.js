...
import { register, login, getProfile, updateProfile, updateAddress } from "./userController.js";
...
router.post("/upload", auth, upload.single("photo"), catchAsyncError(async (req, res, next) => {
    if (!req.file) {
...
router.get("/checklogin", auth, catchAsyncError(async (req, res, next) => {
...
router.put("/add-address", auth, catchAsyncError(async (req, res, next) => {
    console.log("hello");
    let userId = req.user_id;
    if (!userId) {
        return next(new Errorhadler("user id not found", 400));
...
router.post("/address", verifyToken, updateAddress);
